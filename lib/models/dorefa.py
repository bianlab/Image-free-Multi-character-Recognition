...
class Conv2d_Q(nn.Conv2d):
    def __init__(self, in_channels, out_channels, kernel_size, w_bit, a_bit, stride=1,
                 padding=0, dilation=1, groups=1, bias=True):
        super(Conv2d_Q, self).__init__(in_channels, out_channels, kernel_size, stride,
                                     padding, dilation, groups, bias)
        self.w_bit = w_bit
        self.quantize_fn = weight_quantize_fn(w_bit=w_bit)
        self.act_q = activation_quantize_fn(a_bit=a_bit)

    def forward(self, input, order=None):
        weight_q = self.quantize_fn(self.weight)
        input_q = self.act_q(input)
...
class Linear_Q(nn.Linear):
    def __init__(self, in_features, out_features, w_bit, a_bit, bias=True):
